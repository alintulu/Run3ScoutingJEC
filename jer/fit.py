...
def lin_fit(ax, x, y, sd_y, xlabel="x [ux]", ylabel="y [uy]", xm=0., xM=1., ym=0., yM=1., 
            verbose=False, plot=False, label="Scouting", title="Barrel", colour="black", setrange=False):
    ...
    if (plot):
        ...
        ax.errorbar(x, y, yerr=sd_y, xerr=0, ls='', marker='o' if label == "Scouting" else "^", markersize=10., 
                         color=colour, label=label) # + f" (y(0) = {c:.3f}" + r" $\pm$ " + f"{np.sqrt(var_c):.3f})")
        ...
        ax.plot(x1, y1, linestyle='solid' if label == "Scouting" else "dashed", color=colour)
        ...
        #ax.plot(x1,y1_plus_1sigma, linestyle='-', color="lightgray") #, label=r'Fit $\pm 1\sigma$')
        #ax.plot(x1,y1_minus_1sigma, linestyle='-', color="lightgray")
